...
int stModify(stList* ST, stData manE, stOpt manO, obOpt objO) {
	...
	if (ST->total == 0) {
		...
		for (i = 0; i < ST->total; i++) {
			if (strcmp(stTemp->Einfo.stnum, manE.stnum) == 0) {
				...
				strcpy(stTemp->Minfo.monitor, objO.monitor);
				strcpy(stTemp->Minfo.monidate, objO.monidate);
				strcpy(stTemp->Minfo.mouse, objO.mouse);
				strcpy(stTemp->Minfo.mousdate, objO.mousdate);
				strcpy(stTemp->Minfo.keyboard, objO.keyboard);
				strcpy(stTemp->Minfo.keydate, objO.keydate);
